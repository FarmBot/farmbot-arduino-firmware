...
void loadTMC2130ParametersMotor(TMC2130_Basics *tb, int microsteps, int current, int sensitivity, bool stealth)
{
  ...
  if (stealth)
  {
    ...
    // Enable stealth
    tb->set_PWMCONF(FB_TMC_PWMCONF_PWM_AUTOSCALE, 1);
    tb->set_PWMCONF(FB_TMC_PWMCONF_PWM_FREQ, 0);
    tb->set_PWMCONF(FB_TMC_PWMCONF_PWM_GRAD, 1);
    tb->set_PWMCONF(FB_TMC_PWMCONF_PWM_AMPL, 128);
